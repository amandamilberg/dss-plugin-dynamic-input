...
customize_sheet = get_recipe_config()['customize_sheet_names']
sheet_name_list = get_recipe_config()['sheet_names']


input_datasets_names = [name.split('.')[-1] for name in input_datasets_ids]
if len(input_datasets_names) == 0:
    logger.warning("Received no input datasets names. input_datasets_ids={}, input_datasets_names={}".format(input_datasets_ids, input_datasets_names))
...
dataframes_to_xlsx(input_datasets_names, tmp_file_path, lambda name: dataiku.Dataset(name).get_dataframe(),customize_sheet,sheet_name_list)
